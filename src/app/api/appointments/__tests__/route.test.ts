...
describe('GET /api/appointments', () => {
  ...
  it('200 returns appointments for the user', async () => {
    await setClient(createMockClient({
      user: clientUser,
      tables: { appointments: { select: { data: [{ id: 'a1', title: 'Appt', start_time: '2024-02-01' }] } } as TableOps },
    }))
    const { GET } = await import('../route')
    const res = await GET()
...
describe('POST /api/appointments', () => {
  it('401 unauthenticated', async () => {
    ...
    const res = await POST(buildRequest('http://localhost/api/appointments', { method: 'POST', body: validPayload }))
    expect(res.status).toBe(401)
  })
  ...
  it('400 when validation fails (missing title)', async () => {
    await setClient(createMockClient({ user: clientUser }))
    const { POST } = await import('../route')
    const res = await POST(buildRequest('http://localhost/api/appointments', { method: 'POST', body: { startTime: 'x', endTime: 'y' } }))
  ...
  it('201 creates appointment on success', async () => {
    await setClient(createMockClient({
      user: clientUser,
      tables: { appointments: { insert: { data: { id: 'a-1', title: 'Intro' } } } as TableOps },
    }))
    const { POST } = await import('../route')
    const res = await POST(buildRequest('http://localhost/api/appointments', { method: 'POST', body: validPayload }))
  ...
  it('500 when insert fails', async () => {
    await setClient(createMockClient({
      user: clientUser,
      tables: { appointments: { insert: { data: null, error: { message: 'db' } } } as TableOps },
    }))
    const { POST } = await import('../route')
    const res = await POST(buildRequest('http://localhost/api/appointments', { method: 'POST', body: validPayload }))
