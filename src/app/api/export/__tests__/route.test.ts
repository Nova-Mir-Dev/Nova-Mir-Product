...
describe('GET /api/export', () => {
  it('401 unauthenticated', async () => {
    ...
    const res = await GET(
      buildRequest('http://localhost/api/export', { method: 'GET' }),
    )
  ...
  it('400 when entity unknown', async () => {
    ...
    const { GET } = await import('../route')
    const res = await GET(
      buildRequest('http://localhost/api/export?entity=bogus', {
        method: 'GET',
      }),
    )
  ...
  it('200 returns JSON by default', async () => {
    ...
    const { GET } = await import('../route')
    const res = await GET(
      buildRequest('http://localhost/api/export?entity=leads', {
        method: 'GET',
      }),
    )
  ...
  it('200 returns CSV when format=csv', async () => {
    ...
    const { GET } = await import('../route')
    const res = await GET(
      buildRequest('http://localhost/api/export?entity=leads&format=csv', {
        method: 'GET',
      }),
    )
    expect(res.status).toBe(200)
