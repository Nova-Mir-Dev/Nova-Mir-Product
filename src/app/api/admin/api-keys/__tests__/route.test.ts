...
describe('POST /api/admin/api-keys', () => {
  it('400 when body fails validation', async () => {
    ...
    const res = await POST(buildRequest('http://localhost/api/admin/api-keys', { method: 'POST', body: { name: 123 } }))
  ...
  it('401 unauthenticated', async () => {
    await setClient(createMockClient({ user: unauthUser }))
    const { POST } = await import('../route')
    const res = await POST(buildRequest('http://localhost/api/admin/api-keys', { method: 'POST', body: { name: 'k' } }))
    expect(res.status).toBe(401)
  })
  ...
  it('200 creates a key when admin', async () => {
    await setClient(createMockClient({ user: adminUser, tables: { users: { select: { data: adminProfile } } } }))
    const { POST } = await import('../route')
    const res = await POST(buildRequest('http://localhost/api/admin/api-keys', { method: 'POST', body: { name: 'My Key' } }))
    expect(res.status).toBe(200)
    const body = await res.json()
