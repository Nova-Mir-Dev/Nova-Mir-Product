...
export async function POST(request: Request) {
  ...
  const { allowed } = await rateLimit(`admin:content:portfolio:${user.id}`, 30, 60000)
  if (!allowed) {
  ...
  if (!parsed.success) {
    Sentry.captureMessage('Admin portfolio POST validation failed', { extra: { issues: parsed.error.issues } })
    return NextResponse.json({ error: 'Validation failed.' }, { status: 400 })
  }

  const admin = createServiceClient()
  const { data, error } = await admin.from('portfolio_projects').insert(parsed.data).select().single()
...
export async function PUT(request: Request) {
  ...
  if (!parsed.success) {
    Sentry.captureMessage('Admin portfolio PUT validation failed', { extra: { issues: parsed.error.issues } })
    return NextResponse.json({ error: 'Validation failed.' }, { status: 400 })
  }
  ...
  const admin = createServiceClient()
  const { data, error } = await admin.from('portfolio_projects').update(updates).eq('id', id).select().single()
