...
describe('GET /api/admin/content/portfolio', () => {
  it('401 unauthenticated', async () => {
    ...
    const res = await GET(
      buildRequest('http://localhost/api/admin/content/portfolio', {
        method: 'GET',
      }),
    )
  ...
  it('200 returns projects', async () => {
    await ctxAdmin({
      portfolio_projects: {
        select: { data: [{ id: 'p1', title: 'P', is_published: true }] },
      },
    })
    const { GET } = await import('../route')
    const res = await GET(
      buildRequest(
        'http://localhost/api/admin/content/portfolio?published=true',
        { method: 'GET' },
      ),
    )
  ...
  it('500 when fetch fails', async () => {
    await ctxAdmin({
      portfolio_projects: { select: { data: null, error: { message: 'db' } } },
    })
    const { GET } = await import('../route')
    const res = await GET(
      buildRequest('http://localhost/api/admin/content/portfolio', {
...
describe('POST /api/admin/content/portfolio', () => {
  ...
  it('201 creates project', async () => {
    await ctxAdmin({
      portfolio_projects: { insert: { data: { id: 'p-1', title: 'P' } } },
    })
    const { POST } = await import('../route')
    const res = await POST(
      buildRequest('http://localhost/api/admin/content/portfolio', {
  ...
  it('500 when insert fails', async () => {
    await ctxAdmin({
      portfolio_projects: { insert: { data: null, error: { message: 'db' } } },
    })
    const { POST } = await import('../route')
    const res = await POST(
      buildRequest('http://localhost/api/admin/content/portfolio', {
...
describe('PUT /api/admin/content/portfolio', () => {
  ...
  it('200 updates project', async () => {
    await ctxAdmin({
      portfolio_projects: { update: { data: { id: 'p-1', title: 'Updated' } } },
    })
    const { PUT } = await import('../route')
    const res = await PUT(
      buildRequest('http://localhost/api/admin/content/portfolio', {
        method: 'PUT',
        body: { id: 'p-1', title: 'Updated' },
      }),
    )
    expect(res.status).toBe(200)
  ...
  it('500 when update fails', async () => {
    await ctxAdmin({
      portfolio_projects: { update: { data: null, error: { message: 'db' } } },
    })
    const { PUT } = await import('../route')
    const res = await PUT(
      buildRequest('http://localhost/api/admin/content/portfolio', {
...
describe('DELETE /api/admin/content/portfolio', () => {
  ...
  it('200 deletes project', async () => {
    await ctxAdmin({
      portfolio_projects: { delete: { data: null, error: null } },
    })
    const { DELETE } = await import('../route')
    const res = await DELETE(
      buildRequest('http://localhost/api/admin/content/portfolio?id=p-1', {
        method: 'DELETE',
      }),
    )
    expect(res.status).toBe(200)
  ...
  it('500 when delete fails', async () => {
    await ctxAdmin({
      portfolio_projects: { delete: { data: null, error: { message: 'db' } } },
    })
    const { DELETE } = await import('../route')
    const res = await DELETE(
      buildRequest('http://localhost/api/admin/content/portfolio?id=p-1', {
