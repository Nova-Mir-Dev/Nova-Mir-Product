...
export async function GET(request: Request) {
  try {
    ...
    const { allowed } = await rateLimit(`admin:leads:${user.id}`, 30, 60000)
    if (!allowed) {
    ...
    let query = admin
      .from('leads')
      .select(
        'id, name, email, business_name, phone, service_interest, budget_range, message, timeline, referral_source, status, source, notes, created_at',
      )
