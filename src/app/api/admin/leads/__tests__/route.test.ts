...
describe('GET /api/admin/leads', () => {
  it('401 unauthenticated', async () => {
    ...
    const res = await GET(
      buildRequest('http://localhost/api/admin/leads', { method: 'GET' }),
    )
  ...
  it('200 returns leads with status filter', async () => {
    ...
    const admin = createMockClient({
      tables: {
        leads: { select: { data: [{ id: 'l1', status: 'new' }] } } as TableOps,
      },
    })
    await setClients(server, admin)
    const { GET } = await import('../route')
    const res = await GET(
      buildRequest('http://localhost/api/admin/leads?status=new', {
        method: 'GET',
      }),
    )
...
describe('POST /api/admin/leads', () => {
  it('401 unauthenticated', async () => {
    ...
    const res = await POST(
      buildRequest('http://localhost/api/admin/leads', {
        method: 'POST',
        body: createLeadBody,
      }),
    )
  ...
  it('201 creates lead on success', async () => {
    ...
    const admin = createMockClient({
      tables: {
        leads: {
          insert: {
            data: { id: 'l2', name: 'Jane', email: 'jane@example.com' },
          },
        } as TableOps,
      },
    })
  ...
  it('500 when insert fails', async () => {
    ...
    const admin = createMockClient({
      tables: {
        leads: { insert: { data: null, error: { message: 'db' } } } as TableOps,
      },
    })
    await setClients(server, admin)
    const { POST } = await import('../route')
...
describe('PATCH /api/admin/leads', () => {
  ...
  it('200 updates lead', async () => {
    ...
    const admin = createMockClient({
      tables: {
        leads: { update: { data: { id: 'l1', status: 'won' } } } as TableOps,
      },
    })
    await setClients(server, admin)
    const { PATCH } = await import('../route')
  ...
  it('500 when update fails', async () => {
    ...
    const admin = createMockClient({
      tables: {
        leads: { update: { data: null, error: { message: 'db' } } } as TableOps,
      },
    })
    await setClients(server, admin)
    const { PATCH } = await import('../route')
