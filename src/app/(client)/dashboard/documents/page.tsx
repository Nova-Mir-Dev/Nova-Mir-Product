  ...
  return (
    <Stack spacing="lg">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <Text element={{ as: "h1", size: "h3" }} weight="semibold">Documents</Text>
      ...
      {error && (
        <Card>
          <Text element={{ size: "sm" }} color="accent">{error}</Text>
        </Card>
      )}
            ...
            <Card key={doc.id}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <Stack spacing="xs">
                  ...
                  <Text element={{ size: "sm" }} color="secondary">Awaiting your signature</Text>
                </Stack>
                <a href={doc.file_url} target="_blank" rel="noopener noreferrer">
                  <Button variant="primary" type="button">Review & Sign</Button>
                </a>
              </div>
            </Card>
          ))}
      ...
      {CATEGORIES.map((cat) => {
        ...
        return (
          <Stack key={cat} spacing="sm">
            <Text element={{ as: "h2", size: "h5" }} weight="semibold">{CATEGORY_LABELS[cat]}</Text>
            {docs.map((doc) => (
              <Card key={doc.id}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <Stack spacing="xs">
                  ...
                  <a href={doc.file_url} target="_blank" rel="noopener noreferrer">
                    <Button variant="tertiary" type="button">Download</Button>
                  </a>
                </div>
              </Card>
            ))}
      ...
      {uncategorized.length > 0 && (
        <Stack spacing="sm">
          <Text element={{ as: "h2", size: "h5" }} weight="semibold">All Documents</Text>
