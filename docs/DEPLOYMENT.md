# Deployment Guide — nova-mir-product

This guide covers everything you need to deploy this project manually.
Completed items can be checked off as you go.

## Prerequisites

- [ ] Node.js 20+ installed
- [ ] npm 10+ installed
- [ ] Git repository connected to GitHub

## 1. Environment Variables

Copy `.env.example` to `.env.local` and fill in every variable:

```bash
cp .env.example .env.local
```

### Supabase Setup

- [ ] Create a project at [supabase.com/dashboard](https://supabase.com/dashboard)
- [ ] Go to Project Settings > API
- [ ] Copy `Project URL` → `NEXT_PUBLIC_SUPABASE_URL`
- [ ] Copy `anon public` key → `NEXT_PUBLIC_SUPABASE_ANON_KEY`
- [ ] Copy `service_role` secret → `SUPABASE_SERVICE_ROLE_KEY` (never expose to client!)

### Sentry Setup

- [ ] Create a project at [sentry.io](https://sentry.io)
- [ ] Copy DSN → `NEXT_PUBLIC_SENTRY_DSN` and `SENTRY_DSN`
- [ ] Go to Settings > Developer Settings > Auth Tokens
- [ ] Create token with `project:releases` scope → `SENTRY_AUTH_TOKEN`
- [ ] Set `SENTRY_ORG` and `SENTRY_PROJECT` for source map uploads

### Slack App Setup

- [ ] Go to [api.slack.com/apps](https://api.slack.com/apps) → Create New App
- [ ] Enable Socket Mode if using `slack-app/` standalone
- [ ] Set `SLACK_BOT_TOKEN` from OAuth & Permissions
- [ ] Set `SLACK_SIGNING_SECRET` from Basic Information
- [ ] Configure Event Subscriptions URL: `https://YOUR_DOMAIN/api/slack/events`

## 2. Database

- [ ] Run migrations in Supabase SQL Editor or via CLI:

```bash
npx supabase db push
```

#### Running Migrations

The project uses forward-only SQL migrations in `supabase/migrations/`. To apply:

1. Go to Supabase Dashboard → SQL Editor
2. Open each migration file in `supabase/migrations/` in order (oldest first)
3. Execute the SQL
4. Verify with: `SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public'` (should show 25+ tables)

Alternatively, use the Supabase CLI:
```bash
# Install Supabase CLI
brew install supabase/tap/supabase

# Link to your project
supabase link --project-ref <your-project-ref>

# Apply all pending migrations
supabase db push
```

#### Seed Data

After migrations, you need seed data for managed content tables. Create an admin user first via the UI at `/admin/auth/sign-up`, then populate these tables via the admin panel:

- `pricing_tiers` — Set your pricing page content
- `portfolio_projects` — Add portfolio items
- `hero_headlines` — Add rotating hero headlines
- `public_nav_links` — Configure site navigation
- `testimonials` — Add client testimonials
- `process_steps` — Configure "how it works" steps

- [ ] Verify tables created: `users`, `audit_logs`, `api_keys`, `appointments`,

## 3. Hosting

### Deploy to Vercel

- [ ] Connect your Git repository at [vercel.com](https://vercel.com)
- [ ] Import project — Vercel auto-detects Next.js
- [ ] Add all environment variables in Project Settings > Environment Variables
- [ ] Deploy — Vercel auto-builds on every push to main
- [ ] Set up custom domain in Project Settings > Domains
- [ ] Enable automatic HTTPS (enabled by default)

## 4. DNS & Domain

- [ ] Point your domain to your hosting provider
- [ ] Verify HTTPS/TLS is working
- [ ] Vercel provides automatic SSL via Let's Encrypt

## 5. CI/CD

- [ ] Push to GitHub — CI runs on every push and PR
- [ ] Add repository secrets in Settings > Secrets and Variables > Actions:
- [ ] Required: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`
- [ ] Dependabot is configured for weekly npm + Actions updates

## 6. Post-Deploy Verification

- [ ] Verify the app loads at your production URL
- [ ] Test authentication flow (sign up, sign in, sign out)
- [ ] Test slack integration
- [ ] Check sentry logs for errors
- [ ] Verify compliance docs are accessible (`docs/COMPLIANCE.md`, etc.)
- [ ] Check `/.well-known/security.txt` is accessible

## 7. Compliance & Legal

### GDPR Requirements

- [ ] Appoint a Data Protection Officer (DPO) — update `docs/PRIVACY.md` with contact
- [ ] Maintain an Article 30 processing register
- [ ] Complete Data Protection Impact Assessment (DPIA) for high-risk processing
- [ ] Sign Data Processing Agreements with all vendors
- [ ] Establish procedure to respond to DSARs within 30 days
- [ ] Establish breach notification procedure (72-hour deadline)

### CCPA/CPRA Requirements

- [ ] Implement 'Do Not Sell or Share My Personal Information' mechanism
- [ ] Document categories of personal information collected
- [ ] Establish procedure to verify consumer identity for deletion/access requests

## 8. Security Checklist

- [ ] All environment variables are set (no placeholder values)
- [ ] API keys and secrets are not in version control
- [ ] HTTPS is enforced for all traffic
- [ ] CSP headers are not in report-only mode
- [ ] IP allowlist is configured with your team's IPs in `ALLOWED_IPS` (optional — set in env if using ip-allowlist.ts)
- [ ] Test all API endpoints with authentication required
- [ ] Set up automated dependency updates (Dependabot/Snyk)
- [ ] Review `docs/SECURITY.md` for complete checklist

## 9. Portal Architecture

This project is one of three connected portals:

```
nova-mir-product  ← YOU ARE HERE (Marketing site + Admin + Client dashboard)
nova-mir-admin    ← Admin-only portal (separate deployment, same DB)
nova-mir-clients  ← Client-only portal (separate deployment, same DB)
```

All three portals share the same Supabase project and database. Each portal:
1. Reads from the same tables
2. Uses the same Supabase URL/anon key
3. Has its own Vercel deployment and domain
4. Routes users based on role (admin vs client)

The Product portal is the primary entry point. The Admin and Clients portals are satellite apps that should be deployed to their own Vercel projects with the same environment variables.

---
Generated by [Project Bootstrapper](https://github.com/rosejas13/project-bootstrapper)
