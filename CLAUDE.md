...
- **i18n**: next-intl, locales `en`/`es`/`ru`. `docs/I18N.md` is the authority — read it before touching `messages/*.json`. es/ru are machine drafts pending native review; don't promote them to marketing surfaces before that pass. See AGENTS.md "Internationalization Rule".
...
- **Quality contract**: the 14-rule contract lives in `~/.config/opencode/agent-skills/references/quality-standards.md` — that file is the source of truth; don't restate the rules here. Run `skill { name: "full-audit" }` before releases.
- **Must-use skills**: AGENTS.md "Must-Use Skills" is the maintained list.
